  ...
  if(isset($_POST['update_profile']))
{
  ...
  // Phone number already exists check (other users)
  ...
  if(isset($_FILES['profile']) && $_FILES['profile']['size'] > 0){
    ...
    // Old picture delete
    $old_res  = select("SELECT `profile` FROM `user_cred` WHERE `id`=? LIMIT 1", [$user_id], "i");
    $old_user = mysqli_fetch_assoc($old_res);
    deleteImage($old_user['profile'], USERS_FOLDER);

    $profile_update = ", `profile`='$img'";

    // Session update
    $_SESSION['uPic'] = $img;
  }

  $upd = mysqli_query($con, "UPDATE `user_cred` SET 
    `name`='$data[name]',
...
if(isset($_POST['update_password']))
{
  ...
  $upd = mysqli_query($con, "UPDATE `user_cred` SET `password`='$enc_pass' WHERE `id`='$user_id'");
