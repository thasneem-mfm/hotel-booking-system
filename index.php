...
<div class="alert alert-success alert-dismissible fade show m-3" role="alert">
  <strong>🎉 Booking Confirmed!</strong> Your booking has been received successfully. A confirmation email has been sent to your email address.
     ...
     while($room_data = mysqli_fetch_assoc($room_res))
     {
      ...
      $fea_q = mysqli_query($con,"SELECT f.name FROM `features`f 
        ...
        $fac_q = mysqli_query($con,"SELECT f.name FROM `facilities`f 
         ...
         $avg_r    = round($avg_data['avg_rating']);
         $total_r  = $avg_data['total'];

         $stars_r = '';
         for($s = 1; $s <= 5; $s++){
           if($s <= $avg_r){
             $stars_r .= '<i class="bi bi-star-fill text-warning"></i>';
           } else {
             $stars_r .= '<i class="bi bi-star text-warning"></i>';
           }
         }
          ...
          <div class="card border-0 shadow" style="max-width:350px; margin:auto;">
           ...
           <div class="card-body">
             ...
             <div class="rating mb-4 mt-4">
              ...
              <small class="text-muted ms-1">($total_r)</small>
             </div>
...
<div class="container mt-5">
  <div class="swiper swiper-testimonials">
    <div class="swiper-wrapper mb-5">
      ...
      <?php
        $testimonial_res = mysqli_query($con, "SELECT rv.*, uc.name as user_name, uc.profile as user_pic, r.name as room_name
          FROM `reviews` rv
          JOIN `user_cred` uc ON rv.user_id = uc.id
          JOIN `rooms` r ON rv.room_id = r.id
          WHERE rv.status = 1
          ORDER BY rv.id DESC LIMIT 6");

        if(mysqli_num_rows($testimonial_res) == 0){
          // No reviews yet — default slide
          echo "
          <div class='swiper-slide bg-white p-4'>
            <div class='profile d-flex align-items-center mb-3'>
              <img src='images/facilities/IMG_55536.svg' width='80px'>
              <h6 class='m-0 ms-2'>No reviews yet</h6>
            </div>
            <p>Be the first to review our rooms!</p>
          </div>
          ";
        }

        while($tv = mysqli_fetch_assoc($testimonial_res)){
          
          $stars_t = '';
          for($s = 1; $s <= 5; $s++){
            if($s <= $tv['rating']){
              $stars_t .= '<i class="bi bi-star-fill text-warning"></i>';
            } else {
              $stars_t .= '<i class="bi bi-star text-warning"></i>';
            }
          }

          $user_pic_t  = USERS_IMG_PATH.$tv['user_pic'];
          $user_name_t = $tv['user_name'];
          $review_t    = $tv['review'];
          $room_name_t = $tv['room_name'];

          echo "
          <div class='swiper-slide bg-white p-4'>
            <div class='profile d-flex align-items-center mb-3'>
              <img src='$user_pic_t' width='60px' height='60px' style='object-fit:cover;' class='rounded-circle'>
              <div class='ms-2'>
                <h6 class='m-0'>$user_name_t</h6>
                <small class='text-muted'>$room_name_t</small>
              </div>
            </div>
            <p>$review_t</p>
            <div class='rating'>
...
<div class="modal fade" id="recoveryModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
 var swiper = new Swiper(".swiper-container", {
   ...
   document.querySelector('.swiper-container').onclick = () => {
   swiper.slideNext();
 };
